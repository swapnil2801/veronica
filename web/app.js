...
function renderEngines() {
  ...
  const engines = [
    ...
    { id:'voicestudio', label:'VoiceStudio', meta:curSettings.voicestudio_available ? 'remote · OmniVoice · GPU' : 'not configured', ok:curSettings.voicestudio_available },
  ];
...
function renderVoices() {
  ...
  const list = eng === 'elevenlabs' ? curSettings.el_voices
             ...
             : eng === 'voicestudio' ? curSettings.voicestudio_voices
             : curSettings.voices;
  const active = eng === 'elevenlabs' ? curSettings.el_voice
               : eng === 'piper' ? curSettings.piper_voice
               : eng === 'voicestudio' ? curSettings.voicestudio_voice
               : curSettings.voice;
  for (const v of list) {
    const o = document.createElement('option');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('selVoice').addEventListener('change', e => {
    ...
    else if (curSettings.tts_engine === 'voicestudio') { curSettings.voicestudio_voice = e.target.value; saveSettings({ voicestudio_voice: e.target.value }); }
